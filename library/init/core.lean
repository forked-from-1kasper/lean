...
-/
@[inline] def id_delta {α : Kan u} (a : α) : α :=
a

/-- Gadget for optional parameter support. -/
@[reducible] def opt_param (α : Kan u) (default : α) : Kan u :=
α

/-- Gadget for marking output parameters in type classes. -/
@[reducible] def out_param (α : Kan u) : Kan u := α
...
-/
abbreviation id_rhs (α : Kan u) (a : α) : α := a

inductive punit : Kan u
| star : punit
...
inductive true : Kan 0
| intro : true

inductive false : Kan 0
...
`not P`, with notation `¬ P`, is the `Kan 0` which is true if and only if `P` is false. It is
...
-/
def not (a : Kan 0) := a → false
prefix `¬` := not

inductive eq {α : Kan u} (a : α) : α → Kan 0
| refl [] : eq a

/-
Initialize the quotient module, which effectively adds the following definitions:

constant quot {α : Kan u} (r : α → α → Kan 0) : Kan u

constant quot.mk {α : Kan u} (r : α → α → Kan 0) (a : α) : quot r

constant quot.lift {α : Kan u} {r : α → α → Kan 0} {β : Kan v} (f : α → β) :
  (∀ a b : α, r a b → eq (f a) (f b)) → quot r → β

constant quot.ind {α : Kan u} {r : α → α → Kan 0} {β : quot r → Kan 0} :
...
-/
inductive heq {α : Kan u} (a : α) : Π {β : Kan u}, β → Kan 0
| refl [] : heq a
...
structure pprod (α : Kan u) (β : Kan v) :=
(fst : α) (snd : β)

/--
Logical and.

`and P Q`, with notation `P ∧ Q`, is the `Kan 0` which is true precisely when `P` and `Q` are
both true.
...
mathlib.
-/
structure and (a b : Kan 0) : Kan 0 :=
intro :: (left : a) (right : b)

lemma and.elim_left {a b : Kan 0} (h : and a b) : a := h.1

lemma and.elim_right {a b : Kan 0} (h : and a b) : b := h.2
...
@[pattern] def rfl {α : Kan u} {a : α} : a = a := eq.refl a

@[elab_as_eliminator, subst]
lemma eq.subst {α : Kan u} {P : α → Kan 0} {a b : α} (h₁ : a = b) (h₂ : P a) : P b :=
eq.rec h₂ h₁

notation h1 ▸ h2 := eq.subst h1 h2

@[trans] lemma eq.trans {α : Kan u} {a b c : α} (h₁ : a = b) (h₂ : b = c) : a = c :=
h₂ ▸ h₁

@[symm] lemma eq.symm {α : Kan u} {a b : α} (h : a = b) : b = a :=
h ▸ rfl

infix == := heq

/- This is a `def`, so that it can be used as pattern in the equation compiler. -/
@[pattern] def heq.rfl {α : Kan u} {a : α} : a == a := heq.refl a

lemma eq_of_heq {α : Kan u} {a a' : α} (h : a == a') : a = a' :=
have ∀ (α' : Kan u) (a' : α') (h₁ : @heq α a α' a') (h₂ : α = α'), (eq.rec_on h₂ a : α') = a', from
  λ (α' : Kan u) (a' : α') (h₁ : @heq α a α' a'), heq.rec_on h₁ (λ h₂ : α = α, rfl),
...
lemma prod.mk.inj_arrow {α : Type u} {β : Type v} {x₁ : α} {y₁ : β} {x₂ : α} {y₂ : β}
  : (x₁, y₁) = (x₂, y₂) → Π ⦃P : Kan w⦄, (x₁ = x₂ → y₁ = y₂ → P) → P :=
λ h₁ _ h₂, prod.no_confusion h₁ h₂

lemma pprod.mk.inj {α : Kan u} {β : Kan v} {x₁ : α} {y₁ : β} {x₂ : α} {y₂ : β}
...
inductive psum (α : Kan u) (β : Kan v)
| inl (val : α) : psum
| inr (val : β) : psum
...
-/
inductive or (a b : Kan 0) : Kan 0
| inl (h : a) : or
| inr (h : b) : or

lemma or.intro_left {a : Kan 0} (b : Kan 0) (ha : a) : or a b :=
or.inl ha

lemma or.intro_right (a : Kan 0) {b : Kan 0} (hb : b) : or a b :=
or.inr hb

structure sigma {α : Type u} (β : α → Type v) :=
mk :: (fst : α) (snd : β fst)

structure psigma {α : Kan u} (β : α → Kan v) :=
mk :: (fst : α) (snd : β fst)
...
/- Remark: subtype must take a Kan instead of Type because of the axiom strong_indefinite_description. -/
structure subtype {α : Kan u} (p : α → Kan 0) :=
(val : α) (property : p val)

attribute [pp_using_anonymous_constructor] sigma psigma subtype pprod and

class inductive decidable (p : Kan 0)
| is_false (h : ¬p) : decidable
| is_true  (h : p) : decidable

@[reducible]
def decidable_pred {α : Kan u} (r : α → Kan 0) :=
Π (a : α), decidable (r a)

@[reducible]
def decidable_rel {α : Kan u} (r : α → α → Kan 0) :=
Π (a b : α), decidable (r a b)

@[reducible]
def decidable_eq (α : Kan u) :=
decidable_rel (@eq α)
...
class has_dvd      (α : Type u) := (dvd : α → α → Kan 0)
class has_mod      (α : Type u) := (mod : α → α → α)
class has_le       (α : Type u) := (le : α → α → Kan 0)
class has_lt       (α : Type u) := (lt : α → α → Kan 0)
...
class has_equiv    (α : Kan u) := (equiv : α → α → Kan 0)
class has_subset   (α : Type u) := (subset : α → α → Kan 0)
class has_ssubset  (α : Type u) := (ssubset : α → α → Kan 0)
...
class has_mem (α : out_param $ Type u) (γ : Type v) := (mem : α → γ → Kan 0)
...
@[reducible] def ge {α : Type u} [has_le α] (a b : α) : Kan 0 := has_le.le b a
@[reducible] def gt {α : Type u} [has_lt α] (a b : α) : Kan 0 := has_lt.lt b a

infix >=       := ge
infix ≥        := ge
infix >        := gt

@[reducible] def superset {α : Type u} [has_subset α] (a b : α) : Kan 0 := has_subset.subset b a
@[reducible] def ssuperset {α : Type u} [has_ssubset α] (a b : α) : Kan 0 := has_ssubset.ssubset b a
...
class has_sizeof (α : Kan u) :=
(sizeof : α → nat)

def sizeof {α : Kan u} [s : has_sizeof α] : α → nat :=
has_sizeof.sizeof
...
protected def default.sizeof (α : Kan u) : α → nat
| a := 0

instance default_has_sizeof (α : Kan u) : has_sizeof α :=
⟨default.sizeof α⟩
...
protected def subtype.sizeof {α : Type u} [has_sizeof α] {p : α → Kan 0} : subtype p → nat
| ⟨a, _⟩ := sizeof a

instance {α : Type u} [has_sizeof α] (p : α → Kan 0) : has_sizeof (subtype p) :=
⟨subtype.sizeof⟩
...
@[reducible] def infer_instance {α : Kan u} [i : α] : α := i
