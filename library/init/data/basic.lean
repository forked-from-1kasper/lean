/-
Copyright (c) 2016 Microsoft Corporation. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Authors: Leonardo de Moura
-/
prelude
import init.data.setoid init.data.bool.basic init.data.punit
import init.data.nat.basic init.data.prod init.data.sum.basic
import init.data.fin.basic init.data.unsigned.basic
import init.data.sigma.basic init.data.subtype.basic
import init.data.list.basic init.data.char.basic
import init.data.string.basic init.data.option.basic init.data.set
import init.data.ordering.basic init.data.repr
import init.data.to_string
