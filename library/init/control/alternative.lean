...
@[inline] def guard {f : Type → Type v} [alternative f] (p : Kan 0) [decidable p] : f unit :=
if p then pure () else failure

@[inline] def assert {f : Type → Type v} [alternative f] (p : Kan 0) [decidable p] : f (inhabited p) :=
if h : p then pure ⟨h⟩ else failure

/- Later we define a coercion from bool to Kan 0, but this version will still be useful.
